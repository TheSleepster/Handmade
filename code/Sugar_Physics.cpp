...
{ 
    real32 MaxProduct = vDot(Direction, A->Vertices[0]);
    vec2 Result = {};
    for(int Point = 1; 
        Point < A->VertexCount; 
        ++Point) 
...
{
    vec2 AMax = FurthestPoint(A, Direction);
    vec2 BMax = FurthestPoint(B, -Direction);
    vec2 Difference = AMax - BMax;
